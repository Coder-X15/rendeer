        ...
        if dist != 0:
            try:
                imgpixels[pixel[0], pixel[1]] = colorLerp(
                    startColor, endColor, distance([pixel[0], pixel[1], 0], [start[0], start[1], 0]) / dist
                )
            except IndexError:
                noname = "wut"
        else:
...
for key in animationScript:
    ...
    for current in range(numOfTimes):
        ...
        for item in key:
            ...
            for att in item:
                pointy = 1 - intersection.computeIntersections([curves[bezierNumber][0][0], curves[bezierNumber][1][0], curves[bezierNumber][2][0], curves[bezierNumber][3][0]], [curves[bezierNumber][0][1], curves[bezierNumber][1][1], curves[bezierNumber][2][1], curves[bezierNumber][3][1]], [time, time], [-1, 2])[0][1]
                if "+" in att:
                    att = att.split("+")
                    if att[0] == "xRotation":
                        xRotation = ((xRotation + float(att[1])) if repeatMode == "times" else (xRotation + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((xRotationTemp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (xRotationTemp + float(att[1]) * pointy))
                    if att[0] == "yRotation":
                        yRotation = ((yRotation + float(att[1])) if repeatMode == "times" else (yRotation + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((yRotationTemp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (yRotationTemp + float(att[1]) * pointy))
                    if att[0] == "zRotation":
                        zRotation = ((zRotation + float(att[1])) if repeatMode == "times" else (zRotation + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((zRotationTemp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (zRotationTemp + float(att[1]) * pointy))
                    if att[0] == "meshPositionX":
                        meshPosition[0] = ((meshPosition[0] + float(att[1])) if repeatMode == "times" else (meshPosition[0] + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((meshPostion0Temp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (meshPostion0Temp + float(att[1]) * pointy))
                    if att[0] == "meshPositionY":
                        meshPosition[1] = ((meshPosition[1] + float(att[1])) if repeatMode == "times" else (meshPosition[1] + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((meshPostion1Temp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (meshPostion1Temp + float(att[1]) * pointy))
                    if att[0] == "meshPositionZ":
                        meshPosition[2] = ((meshPosition[2] + float(att[1])) if repeatMode == "times" else (meshPosition[2] + float(att[1]) / numOfTimes)) if (interpolation == "linear") else ((meshPostion2Temp + float(att[1]) * numOfTimes * pointy) if repeatMode == "times" else (meshPostion2Temp + float(att[1]) * pointy))
                    if att[0] == "resolution":
                        res = ((res + float(att[1]) / 100) if repeatMode == "times" else (res + (float(att[1]) / 100) / numOfTimes)) if (interpolation == "linear") else ((resTemp + (float(att[1]) / 100) * numOfTimes * pointy) if repeatMode == "times" else (resTemp + (float(att[1]) / 100) * pointy))
                ...
                if "=" in att:
                    att = att.split("=")
                    if att[0] == "xRotation":
                        xRotation = ((xRotation + (float(att[1]) - xRotationTemp)) / numOfTimes) if (interpolation == "linear") else (xRotationTemp + (float(att[1]) - xRotationTemp) * pointy)
                    if att[0] == "yRotation":
                        yRotation = ((yRotation + (float(att[1]) - yRotationTemp)) / numOfTimes) if (interpolation == "linear") else (yRotationTemp + (float(att[1]) - yRotationTemp) * pointy)
                    if att[0] == "zRotation":
                        zRotation = ((zRotation + (float(att[1]) - zRotationTemp)) / numOfTimes) if (interpolation == "linear") else (zRotationTemp + (float(att[1]) - zRotationTemp) * pointy)
                    if att[0] == "meshPositionX":
                        meshPosition[0] = ((meshPosition[0] + (float(att[1]) - meshPostion0Temp)) / numOfTimes) if (interpolation == "linear") else (meshPostion0Temp + (float(att[1]) - meshPostion0Temp) * pointy)
                    if att[0] == "meshPositionY":
                        meshPosition[1] = ((meshPosition[1] + (float(att[1]) - meshPostion1Temp)) / numOfTimes) if (interpolation == "linear") else (meshPostion1Temp + (float(att[1]) - meshPostion1Temp) * pointy)
                    if att[0] == "meshPositionZ":
                        meshPosition[2] = ((meshPosition[2] + (float(att[1]) - meshPostion2Temp)) / numOfTimes) if (interpolation == "linear") else (meshPostion2Temp + (float(att[1]) - meshPostion2Temp) * pointy)
                    if att[0] == "resolution":
                        res = ((res + (float(att[1]) / 100 - resTemp)) / numOfTimes) if (interpolation == "linear") else (resTemp + (float(att[1]) / 100 - resTemp) * pointy)
        ...
        rotatedVerticesXYZ = rotatePoints(verticesPosition, centerPoint, degreesToRadians(yRotation), degreesToRadians(xRotation), degreesToRadians(zRotation))
        ...
        sortedDistances = [[distance_squared(cen, cameraPosition), c] for c, cen in enumerate(faceCenters)]
        ...
        for face in tqdm(mesh3DFaces, ascii=True, desc="Calculating face normals"):
            ...
            faceNormals.append(
                [faceNormal[0] + faceCenters[k][0], faceNormal[1] + faceCenters[k][1], faceNormal[2] + faceCenters[k][2]]
            )
        ...
        for normal in tqdm(faceNormals, ascii=True, desc="Calculating face colors"):
            lightNormalAngle = angleBetweenVectors(
                [normal[0] - faceCenters[j][0], normal[1] - faceCenters[j][1], normal[2] - faceCenters[j][2]], lightVector[0]
            )
        ...
        for face in tqdm(meshFaces, ascii=True, desc="Rendering faces"):
            if (facesDrawn[i] == 1 and settings.backfaceCulling == True) or settings.backfaceCulling == False:
                if shadingMode == settings.ShadingMode.unlit or shadingMode == settings.ShadingMode.flatDiffuse:
                    ...
                    draw.polygon(
                        ...
                        outline=(settings.wireframeColor[0], settings.wireframeColor[1], settings.wireframeColor[2]), fill=(0, 0, 0)
                    )
                elif shadingMode == settings.ShadingMode.gouraud:
                    b = 0
                    dist1 = distance_squared([face[1][0], face[1][1], 0], [face[0][0], face[0][1], 0])
                    dist2 = distance_squared([face[1][0], face[1][1], 0], [face[2][0], face[2][1], 0])
                    dist3 = distance_squared([face[0][0], face[0][1], 0], [face[2][0], face[2][1], 0])
                    if dist1 > dist2:
                        while b < 1:
                            draw_line(
                                ...
                                colorLerp(
                                    vertexColors[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                    vertexColors[rotatedVerticesXYZ.index(mesh3DFaces[i][0])],
                                    b,
                                ),
                                colorLerp(
                                    vertexColors[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                    vertexColors[rotatedVerticesXYZ.index(mesh3DFaces[i][2])],
                                    b,
                                ),
                            )
                            try:
                    ...
                    while o < 1:
                        normals10.append( 
                            vectorLerp(
                                softVertexNormals[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                softVertexNormals[rotatedVerticesXYZ.index(mesh3DFaces[i][0])],
                                o,
                            )
                        )
                        vertices10.append(
                            vectorLerp(
                                rotatedVerticesXYZ[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                rotatedVerticesXYZ[rotatedVerticesXYZ.index(mesh3DFaces[i][0])],
                                o,
                            )
                        )
                    ...
                    while o < 1:
                        normals12.append( 
                            vectorLerp(
                                softVertexNormals[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                softVertexNormals[rotatedVerticesXYZ.index(mesh3DFaces[i][2])],
                                o,
                            )
                        )
                        vertices12.append(
                            vectorLerp(
                                rotatedVerticesXYZ[rotatedVerticesXYZ.index(mesh3DFaces[i][1])],
                                rotatedVerticesXYZ[rotatedVerticesXYZ.index(mesh3DFaces[i][2])],
                                o,
                            )
                        )
            ...
            if renderFaceNormals:
                draw.line(
                    [faceCentersScreen[i][0], faceCentersScreen[i][1], faceNormalsOnScreen[i][0], faceNormalsOnScreen[i][1]],
                    (160, 0, 0),
                )
            i += 1
        ...
        if renderMeshCenter:
            draw.ellipse(
                (newCenterPoint[0] - 1, newCenterPoint[1] - 1, newCenterPoint[0] + 1, newCenterPoint[1] + 1), fill="gray"
            )

        if renderLightSource:
            draw.ellipse(
                (gridCenterOnScreen[0] - 2, gridCenterOnScreen[1] - 2, gridCenterOnScreen[0] + 2, gridCenterOnScreen[1] + 2),
                fill="lightgray",
            )
